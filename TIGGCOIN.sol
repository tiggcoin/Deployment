...
contract AccessControl is Context {
    ...
    function revokeRole(bytes32 role, address account) public virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "AccessControl: sender must be admin to revoke");
...
contract ERC20 is Context {
    ...
    function allowance(address owner_, address spender) public view virtual returns (uint256) { return _allowances[owner_][spender]; }
    ...
    function transfer(address to, uint256 amount) public virtual returns (bool) { _transfer(_msgSender(), to, amount); return true; }
...
contract TIGGCOIN is ERC20, AccessControl, Ownable {
    ...
    uint256 public constant TRANCHE_AMOUNT = 1_000_000_000 * 10**18;
    uint256 public constant MAX_SUPPLY = 10_000_000_000 * 10**18;
    uint256[] public scheduledMintsUTC;
    ...
    constructor() ERC20("TIGGCOIN", "TIGG") Ownable(msg.sender) {
        ...
        scheduledMintsUTC.push(1916956800); // 2030-09-30
        scheduledMintsUTC.push(2074723200); // 2035-09-30
        scheduledMintsUTC.push(2232576000); // 2040-09-30
        scheduledMintsUTC.push(2390342400); // 2045-09-30
        scheduledMintsUTC.push(2548108800); // 2050-09-30
        scheduledMintsUTC.push(2705875200); // 2055-09-30
        scheduledMintsUTC.push(2863728000); // 2060-09-30
        scheduledMintsUTC.push(3021494400); // 2065-09-30
        scheduledMintsUTC.push(3179260800); // 2070-09-30

        for (uint256 i = 0; i < scheduledMintsUTC.length; i++) {
            emit MintScheduled(TRANCHE_AMOUNT, scheduledMintsUTC[i]);
        }
    }

    function decimals() public pure override returns (uint8) { return _DECIMALS; }
    function emergencyStop() external onlyOwner { emergencyStopped = true; emit EmergencyStopped(_msgSender(), block.timestamp); }
    function liftEmergencyStop() external onlyOwner { emergencyStopped = false; emit EmergencyLifted(_msgSender(), block.timestamp); }
    function executeScheduledMint() external onlyRole(MINTER_ROLE) {
        require(!emergencyStopped, "TIGG: emergency stopped");
        require(nextMintIndex < scheduledMintsUTC.length, "TIGG: no scheduled mints left");
        uint256 unlockTime = scheduledMintsUTC[nextMintIndex];
        require(block.timestamp >= unlockTime, "TIGG: scheduled mint not yet available");
        require(totalSupply() + TRANCHE_AMOUNT <= MAX_SUPPLY, "TIGG: max supply exceeded");
        _mint(owner(), TRANCHE_AMOUNT);
        emit MintExecuted(owner(), TRANCHE_AMOUNT, block.timestamp, totalSupply());
        nextMintIndex += 1;
    }
    function scheduledMintsRemaining() external view returns (uint256) { return nextMintIndex >= scheduledMintsUTC.length ? 0 : scheduledMintsUTC.length - nextMintIndex; }
    ...
    function rescueERC20(address token, address to, uint256 amount) external onlyOwner { IERC20(token).safeTransfer(to, amount); emit TokenRescued(token, to, amount); }
    function rescueAllERC20(address token, address to) external onlyOwner { uint256 bal = IERC20(token).balanceOf(address(this)); IERC20(token).safeTransfer(to, bal); emit TokenRescued(token, to, bal); }
    function rescueNative(address payable to, uint256 amount) external onlyOwner { (bool success, ) = to.call{value: amount}(""); require(success, "TIGG: native transfer failed"); emit NativeRescued(to, amount); }
    function rescueAllNative(address payable to) external onlyOwner { uint256 bal = address(this).balance; (bool success, ) = to.call{value: bal}(""); require(success, "TIGG: native transfer failed"); emit NativeRescued(to, bal); }
    function _update(address from, address to, uint256 amount) internal override { require(!emergencyStopped, "TIGG: emergency stopped"); super._update(from, to, amount); }
